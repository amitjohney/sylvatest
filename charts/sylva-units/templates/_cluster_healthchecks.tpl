...
Wait for Cluster resource (except for CAPO, see why below):

*/}}

{{ if not ($cluster.capi_providers.infra_provider | eq "capo") }}
...
Wait for infra provider

When the infra provider is capo, instead of waiting for the Cluster object
we wait on the OpenStackCluster object, because we have a workaround for issue
https://gitlab.com/sylva-projects/sylva-core/-/issues/309 that consists in
pausing the Cluster object, which after that does not become "Ready".
