...
{{- define "interpret-inner-gotpl" -}}
    ...
    {{- else if (eq $kind "map") -}}
        ...
        {{- range $key := keys $data | sortAlpha -}}
            ...
            {{- $tpl_value := index (tuple $envAll (index $data $key) | include "interpret-inner-gotpl" | fromJson) "result" -}}
            {{- $_ := unset $data $key -}}
            {{- if (eq (kindOf $tpl_value) "string") -}}
                {{- if (regexMatch "^( |\n)*{\"encapsulated-result\":" $tpl_value) -}}
                    {{- $_ := set $data $tpl_key (index (fromJson $tpl_value) "encapsulated-result") -}}
                {{- else if (ne $tpl_value "skip-as-set-only-if-result-was-false") -}}
                    {{- $_ := set $data $tpl_key $tpl_value -}}
                {{- end -}}
            {{- else -}}
                {{- $_ := set $data $tpl_key $tpl_value -}}
            {{- end -}}
