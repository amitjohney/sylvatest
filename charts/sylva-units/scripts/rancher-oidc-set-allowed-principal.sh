...
# Set this UUID to the keycloakoidc authConfig to make sylva-admin allowed even in more restricted modes
kubectl patch authconfig keycloakoidc --type merge -p '{"allowedPrincipalIds":["keycloakoidc_user://'$sylva_admin_uuid'"]}'

kubectl get user $admin_user -o yaml
kubectl get authconfig keycloakoidc -o yaml
