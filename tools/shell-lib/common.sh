...
SYLVA_TOOLBOX_VERSION=${SYLVA_TOOLBOX_VERSION:-"v0.3.17"}
...
function reconcile_sylva_units() {
  ...
  echo "force reconciliation of sylva-units HelmRelease..."
  ...
  #    which can't give a relevant result it is starts before all Kustomization are the new ones)
  #
  FORCE_RECONCILE_ANNOTATION=""
  if (kubectl get -n sylva-units-preview hr sylva-units -o yaml | yq -e '.status.conditions[] | select(.type=="Ready") | .reason == "InstallFailed"' >/dev/null); then
  ...
  # we wait here to ensure that the HelmRelease controller has started looking at the HelmRelease
  kubectl -n $namespace wait HelmRelease sylva-units \--timeout 30s \
    --for="jsonpath={.status.lastHandledReconcileAt}=$RECONCILE_REQUEST_DATE"

  sylvactl watch -n $namespace HelmRelease/$namespace/sylva-units --timeout 60s
}
...
EOF
  ...
  echo "Check if sylva-units Helm release is ready"
  if ! kubectl wait --for condition=Ready --timeout 0s -n sylva-units-preview HelmRelease sylva-units; then
