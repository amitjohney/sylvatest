...
# in particular that they use _patches, _components, _postRenderers and _substituteFrom
# instead of their equivalents without the '_'
if [[ $HELM_NAME == "sylva-units" ]]; then
  value_file=$chart_dir/values.yaml
  echo -e "\e[0Ksection_start:`date +%s`:additional_check_$value_file\r\e[0K--------------- checking patches/components/postRenderers/substituteFrom in $value_file ..."
  ...
  substituteFrom=$(yq eval '[.units | to_entries | .[] | select(.value.kustomization_spec.postBuild.substituteFrom)] | map(.key) | join(", ")' $value_file)
  if [[ -n $substituteFrom ]]; then
    echo "Some units defined in $value_file use kustomization_spec.postBuild.substituteFrom, although they should use kustomization_spec.postBuild._substituteFrom instead: $substituteFrom"
    exit -1
  fi
